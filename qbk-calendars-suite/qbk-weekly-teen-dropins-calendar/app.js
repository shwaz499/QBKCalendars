(() => {
  ...
  const DAY_START_MIN = 14 * 60; // 2:00 PM
  const DAY_END_MIN = 21 * 60; // 9:00 PM
  const SLOT_COUNT = (DAY_END_MIN - DAY_START_MIN) / SLOT_MINUTES; // 14 half-hour rows
  ...
  function formatWeekTitle(weekStartISO) {
    ...
    return `Teen Drop-Ins Week View (${startText} - ${endText})`;
  }
  ...
  function normalizeAdultEvent(raw, weekStartISO) {
    ...
    const isGlowParty = /glow[\s-]*in[\s-]*the[\s-]*dark[\s-]*party/.test(lower);
    const isDropIn = categoryLower.includes("drop-in")
