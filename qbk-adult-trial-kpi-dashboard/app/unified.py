...
ADULT_KPI_HISTORY_START = dt.datetime(2025, 1, 1, tzinfo=dt.timezone.utc)
...
def _row_activity_at(row: dict[str, Any]) -> dt.datetime | None:
    attrs = row.get("attributes") if isinstance(row.get("attributes"), dict) else {}
    return _as_dt(
        attrs.get("time")
        or attrs.get("create_date")
...
def _last_row_activity_at(rows: list[dict[str, Any]]) -> dt.datetime | None:
    for row in reversed(rows):
        parsed = _row_activity_at(row)
        if parsed is not None:
            return parsed
    return None


def _first_page_at_or_after(
    *,
    fetch_page: Any,
    last_page: int,
    cutoff: dt.datetime,
    ...
    low = 1
    high = max(1, last_page)
    answer = high
    while low <= high:
        mid = (low + high) // 2
        rows = fetch_page(mid)
        last_activity = _last_row_activity_at(rows)
...
def sync_daysmart_to_unified(
    ...
    with get_conn(db_path) as conn:
        ...
        # Adult and youth matching depend on the full DaySmart customer set.
        # Limiting to only the most recent customer pages causes older-but-still-relevant
        # historical leads to miss exact phone/email matches even when the account exists.
        for page in range(1, customer_last_page + 1):
            data = first_data if page == 1 else client.list_customers(
        ...
        if first_data:
            registration_page_cache: dict[int, list[dict[str, Any]]] = {1: first_data}

            def fetch_registration_page(page_number: int) -> list[dict[str, Any]]:
                if page_number not in registration_page_cache:
                    registration_page_cache[page_number] = client.list_registrations(
                        page_number=page_number,
                        page_size=page_size,
                    )[0]
                return registration_page_cache[page_number]

            start_page = _first_page_at_or_after(
                fetch_page=fetch_registration_page,
                last_page=registration_last_page,
                cutoff=ADULT_KPI_HISTORY_START,
            )
        else:
        ...
        for page in range(start_page, registration_last_page + 1):
            data = registration_page_cache.get(page)
            if data is None:
        ...
        if first_data:
            event_registration_page_cache: dict[int, list[dict[str, Any]]] = {1: first_data}

            def fetch_event_registration_page(page_number: int) -> list[dict[str, Any]]:
                if page_number not in event_registration_page_cache:
                    event_registration_page_cache[page_number] = client.list_event_registrations(
                        page_number=page_number,
                        page_size=page_size,
                    )[0]
                return event_registration_page_cache[page_number]

            start_page = _first_page_at_or_after(
                fetch_page=fetch_event_registration_page,
                last_page=event_registration_last_page,
                cutoff=ADULT_KPI_HISTORY_START,
            )
        else:
            event_registration_page_cache = {}
            start_page = 1
            event_registration_last_page = 0
        for page in range(start_page, event_registration_last_page + 1):
            data = event_registration_page_cache.get(page)
            if data is None:
