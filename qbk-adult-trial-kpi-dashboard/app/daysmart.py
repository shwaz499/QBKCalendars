...
class DaysmartClient:
    ...
    def _list_endpoint(
        ...
        *,
        ...
        merged_params = {"page[number]": page_number, "page[size]": page_size}
        if params:
            merged_params.update(params)
        payload = self._request(endpoint, params=merged_params)
        return self._list_payload(payload, page_number=page_number)
    ...
    def get_event(self, event_id: int | str) -> dict[str, Any]:
        payload = self._request(f"/api/v1/events/{event_id}")
        data = payload.get("data") if isinstance(payload, dict) else None
        if not isinstance(data, dict):
            raise DaysmartApiError(f"Event {event_id} did not return a valid payload")
        return data
    ...
    def list_registrations(
        ...
        *,
        ...
        request_params = {"page[number]": page_number, "page[size]": page_size}
        if params:
            request_params.update(params)
        payload = self._request("/api/v1/registrations", params=request_params)
        data = payload.get("data") if isinstance(payload, dict) else []
        if not isinstance(data, list):
            data = []
        last_page = (
    ...
    def list_event_registrations(
        ...
        *,
        ...
        if params:
            request_params.update(params)
        payload = self._request("/api/v1/event-registrations", params=request_params)
        data = payload.get("data") if isinstance(payload, dict) else []
        if not isinstance(data, list):
            data = []
        last_page = (
