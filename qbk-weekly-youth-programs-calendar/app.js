(() => {
  ...
  const SLOT_COUNT = (DAY_END_MIN - DAY_START_MIN) / SLOT_MINUTES; // 26 half-hour rows
  ...
  function formatWeekTitle(weekStartISO) {
    ...
    return `Youth Program Week View (${startText} - ${endText})`;
  }
  ...
  function buildGridSkeleton() {
    ...
    els.weekGrid.style.setProperty("--visible-slot-count", String(SLOT_COUNT));
    els.weekGrid.style.setProperty("--visible-track-height", `${trackHeight}px`);
  ...
  function normalizeAdultEvent(raw, weekStartISO) {
    ...
    const categoryLower = String(raw.category || "").toLowerCase();
    const isTargetProgram = /\b(seals|cubs)\b/.test(lower) || /beach[\s-]*lions/.test(lower);
    const isClassLike = categoryLower.includes("class") || /\bclasses?\b/.test(lower);
    if (!isTargetProgram || !isClassLike) return null;

    let title = sourceTitle.replace(/\s{2,}/g, " ").trim();
    let ageBracketOnNewLine = false;
    if (/\bcubs\b/.test(lower)) {
      title = "Cubs\n(6-9)";
      ageBracketOnNewLine = true;
    } else if (/\bseals?\b/.test(lower)) {
      if (/(13\s*\+|13\+|\(13\+\)|13\+\s*y\/o)/.test(lower)) {
        title = "Seals\n(13+)";
        ageBracketOnNewLine = true;
      } else {
        title = "Seals\n(10-12)";
    ...
    } else if (/beach[\s-]*lions/.test(lower)) {
      if (/\bteal\b/.test(lower)) {
        title = "Beach Lions (Teal)";
      } else if (/\byellow\b/.test(lower)) {
        title = "Beach Lions (Yellow)";
      }
    }
  ...
  function renderEvents(rawEvents, weekStartISO) {
    ...
    for (const event of events) {
      ...
      if (event.ageBracketOnNewLine) {
        title.classList.add("week-event-title-age");
      }
